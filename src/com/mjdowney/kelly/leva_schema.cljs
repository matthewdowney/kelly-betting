(ns com.mjdowney.kelly.leva-schema
  "Rewrite of leva.schema with extra hooks to help with [#11][1].

  [1]: https://github.com/mentat-collective/Leva.cljs/issues/11"
  (:require ["leva" :as l]
            [leva.types :as t]
            [goog.object :as o]))
...
(defn on-change-fn
  ...
  (if !state
    (fn k->on-change [k]
      ; Either update the atom with a custom :on-change, or assoc the new k v
      (let [f (get-in schema [k :on-change] (fn [s v] (assoc s k v)))]
...
(defn opts->argv
  ...
  (let [k->on-change  (on-change-fn atom schema)
        ...
        ;; more detail.
        schema        (fn [] (build-schema schema initial-state k->on-change))
